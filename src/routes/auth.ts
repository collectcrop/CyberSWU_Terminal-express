...
interface tkVerified extends express.Request{
  user: User;
}
...
const authenticateToken = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  jwt.verify(token, SECRET, (err, user) => {
    if (err || !user || typeof user === 'string') {
      console.error('Token verify failed:', err?.name || 'Paramenter Missing', err?.message || 'Some of the paraments we need is missing')
      res.status(403).json({ error: 'Invalid token' })
      return
    }
    (req as tkVerified).user = (user as User) // 把解码后的 user 对象挂到 req 上
    next()
  })
}
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      res.status(401).json({ error: '用户或密码错误' })
      return
    }
...
router.post('/forgot-password', async (req, res) => {   // 忘记密码
  const bd: fgtPass = req.body
  const { email } = bd
  try {
    const user = await pool.query('SELECT * FROM users WHERE email = $1', [email])
    if (user.rows.length === 0) {
      res.status(400).json({ error: '邮箱未注册' })
      return
    }
...
router.post('/reset-password/:token', async (req, res) => {   // 重置密码
  ...
  try {
    ...
    if (result.rows.length === 0) {
      res.status(400).json({ error: '链接已失效或无效' })
      return
    }
...
router.get('/check-admin', authenticateToken, async (req, res) => {
  try {
    const user = (req as tkVerified).user
    const userId = user.id // 从 token 中间件获取
    console.log(userId)
    const result = await pool.query('SELECT role FROM users WHERE id = $1', [userId])
    const role = result.rows[0]?.role
...
router.get('/check-token', authenticateToken, (req, res) => {
  ...
  res.json({
    message: 'Token 有效',
    user: (req as tkVerified).user,  // 包含 id、username、email 等
  })
})
